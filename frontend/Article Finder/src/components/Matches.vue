    ...
    <table class="table" id="resultsTable">
      <tbody>
        ...
        <tr v-for="(article,i) in articles" :key="i">
          <th scope="row">{{i+1+"."}}</th>
          <td>
            <router-link to="/Summary" @click="setCurrentArticle(articles[i].article)" class="nav-link">
            {{ articles[i].article.title }}
            </router-link>
          </td>
          <td>
            <a class="nav-link" v-bind:href="''+articles[i].article.path+''" target="_blank">
