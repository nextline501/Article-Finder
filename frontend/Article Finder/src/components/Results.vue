      ...
      <table class="table" id="resultsTable">
        ...
        <tbody>
          ...
          <tr v-for="(article,i) in dataFromServer" :key="i">
            <th scope="row">{{i+1+"."}}</th>
            <td>
              <router-link to="/Summary" @click="setCurrentArticle(dataFromServer[i].article)" class="nav-link">
              {{ dataFromServer[i].article.title }}
              </router-link>
            </td>
            <td>
              <a class="nav-link" v-bind:href="''+dataFromServer[i].article.path+''" target="_blank">
                <button class="btn-dark">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="16" fill="currentColor" class="bi bi-file-earmark-pdf-fill" viewBox="0 0 16 16">
